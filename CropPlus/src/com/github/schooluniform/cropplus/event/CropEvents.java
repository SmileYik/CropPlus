...
public class CropEvents implements Listener {
  ...
  public void onEntityTrample(EntityInteractEvent e) {
    if (e.getBlock().getType() == getSoil()) {
      ...
      if (CropManager.containsTimer(id)) {
        ...
        CPCropDeathEvent event = new CPCropDeathEvent(
            data.getId(), b.getLocation(), CropDeathEnum.Trample, e.getEntity());
  ...
  public void onPlayerPlantCrop(PlayerInteractEvent e) {
    ...
    if (e.getAction() == Action.PHYSICAL && e.getClickedBlock().getType() == getSoil()) {
      ...
      if (CropManager.containsTimer(id)) {
        ...
        CPCropDeathEvent event = new CPCropDeathEvent(
            data.getId(), b.getLocation(), CropDeathEnum.Trample, e.getPlayer());
        Bukkit.getPluginManager().callEvent(event);
        if (!event.isCancelled()) {
          CropManager.removeTimer(id);
          if (!(b.getState().getData() instanceof Crops)) {
      ...
      return;
    } else if (e.hasItem() && e.getAction() == Action.RIGHT_CLICK_BLOCK
        && e.getClickedBlock().getType() == getSoil()
        && e.getClickedBlock().getRelative(BlockFace.UP).getType() == Material.AIR) {

      // Planting Crop

      if (CropManager.containsTimer(
          CropManager.getID(e.getClickedBlock().getRelative(BlockFace.UP).getLocation()))) {
        if (e.getClickedBlock().getRelative(BlockFace.UP).getType() == Material.AIR) {
          CropManager.removeTimer(
              CropManager.getID(e.getClickedBlock().getRelative(BlockFace.UP).getLocation()));
          e.setCancelled(true);
          return;
        }
      ...
      playerPlantingCrop(name, e);
        ...
        && CropManager.containsTimer(CropManager.getID(e.getClickedBlock().getLocation()))) {
      
      if ((Manager.bukkitVersion < 13 
          && (e.getItem().getType() == Material.valueOf("INK_SACK") 
          && e.getItem().getDurability() == 15)) 
          || e.getItem().getType() == Material.BONE_MEAL) {
        // Bone Meal
        e.setCancelled(true);
  ...
  public void onPlayerBreakCrop(BlockBreakEvent e) {
    ...
    if (CropManager.containsTimer(id)) {
      ...
      if (((Crops) e.getBlock().getState().getData()).getState() == CropState.RIPE) {
        ...
        CPPlayerHarvestingCropEvent cpphce = 
            new CPPlayerHarvestingCropEvent(e.getPlayer(), data.getId(),
            data.getDrops(), e.getBlock().getLocation());
    ...
    } else if (e.getBlock().getType() == getSoil()) {
      Block b = e.getBlock().getRelative(BlockFace.UP);
      ...
      if (CropManager.containsTimer(id)) {
        ...
        if (((Crops) b.getState().getData()).getState() == CropState.RIPE) {
          ...
          /*
           * for(ItemStack item:data.getDrops()) b.getWorld().dropItem(b.getLocation(),
           * item);
           */
        } else {
          CPCropDeathEvent event = 
              new CPCropDeathEvent(data.getId(), b.getLocation(), CropDeathEnum.Break,
              e.getPlayer());
          Bukkit.getPluginManager().callEvent(event);
          if (!event.isCancelled()) {
            e.setCancelled(true);
  ...
  public void onBlockPistonExtend(BlockPistonExtendEvent e) {
    for (Block b : e.getBlocks()) {
      ...
      if (CropManager.containsTimer(id)) {
        ...
        CPCropDeathEvent event = 
            new CPCropDeathEvent(data.getId(), b.getLocation(), CropDeathEnum.PistonExtend);
        Bukkit.getPluginManager().callEvent(event);
        if (!event.isCancelled()) {
          e.setCancelled(true);
          CropManager.removeTimer(id);
  ...
  public void playerPlantingCrop(String name, PlayerInteractEvent e) {
    Block b = e.getClickedBlock().getRelative(BlockFace.UP);
    CPPlayerPlantingCropEvent cpppce = new CPPlayerPlantingCropEvent(
        e.getPlayer(), name, b.getLocation(), e.getClickedBlock().getLocation(), e.getItem());
...
//      bs.setData(new Crops(data.getType().getType()));
//      bs.update(true);
      CropManager.registerTimer(
          new Timer(cpppce.getCropId(), 0, data.getMultiple(), b.getLocation()));
      e.getItem().setAmount(e.getItem().getAmount() - 1);
    }
  ...
  /*
   * public void playerHarvestingCrop(Player p,CropData data,Location
   * location,String id){ CPPlayerHarvestingCropEvent cpphce = new
   * CPPlayerHarvestingCropEvent(p, data.getId(), data.getDrops(), location);
   * Bukkit.getServer().getPluginManager().callEvent(cpphce);
   * if(!cpphce.isCancelled()){ CropManager.removeTimer(id); for(ItemStack item :
   * cpphce.getDrops()) location.getWorld().dropItem(location, item);
